...
Сервер хранит единый срок окончания решения. Каждый следующий участник аукциона получает `AUCTION_SECONDS` (по умолчанию 40 секунд) на ставку или пас. Это отдельный таймер, не `TURN_SECONDS` и не время анимации `TURN_ACTION_SECONDS`. При таймауте сервер сам выполняет пас и передаёт решение следующему участнику либо завершает аукцион. Истёкшее предложение обмена сервер закрывает без передачи денег и имущества. Эти действия выполняются и при отключённых клиентах; такие таймауты не считаются пропусками обычного хода.
...
Клиент отправляет ключ решения, на котором основано действие. Если сервер уже перешёл к другому решению, запоздавшее состояние отклоняется и клиент получает актуальное. Условия отправленного обмена нельзя менять во время рассмотрения. Повторная отправка неизменившегося аукциона не считается новой ставкой и не продлевает срок.
...
- `npm run test:timeouts` проверяет серверный пас, выплату победителю, завершение аукциона, дополнительный бросок, пропуск хода и закрытие обмена без списаний; `test:rooms` дополнительно проверяет истечение решений при отключённом адресате и отклонение старого предложения;
