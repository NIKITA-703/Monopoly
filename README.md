...
### Аукцион и время решения

Сервер хранит единый срок окончания решения. Каждый следующий участник аукциона получает `AUCTION_SECONDS` (по умолчанию 40 секунд) на ставку или пас. Это отдельный таймер, не `TURN_SECONDS` и не время анимации `TURN_ACTION_SECONDS`. При таймауте сервер поручает подключённому клиенту выполнить автоматический пас; такой таймаут не считается пропуском обычного хода.

Клиент сверяет отсчёт с временем из сообщений сервера и измеряет прошедшее время монотонными часами браузера. Разница системных часов игроков не добавляется к таймеру. Срок обновляется сразу при получении сообщения, даже во время анимации. Сетевая задержка может давать небольшое расхождение. Сервер проверяет очередность, доступность ставки и окончание аукциона; отклонённое действие возвращает клиенту сохранённое состояние и действующий срок.
...
- `npm run test:auction` проверяет отсчёт при разных системных часах, очередь ставок и пасов, пропуск недоступных участников и запрет преждевременного завершения аукциона;
